...
"""
The fused rounds of :mod:`discopy.neural.fused` -- the Triton kernels and
the hand-written CUDA kernels of :mod:`discopy.neural.fused_cuda` --
against the reference step of ``CMap._step_body`` on the sudoku lattice
model: the forward and every gradient, the exactness of the hand-written
backward in float64, the fallback on a map the kernels do not serve, and
a training step of the lattice trainer with the fused path on and off.
For the CUDA kernels also: the tensor-core precision against the plain
one and against Triton's, and the cubin cache.

Everything here needs a GPU, triton and cuda-bindings.  Float32
comparisons run at ``"highest"`` matmul precision on both sides, so that
only rounding differs, and on inputs at which no ReLU of the round sits
within ``3e-6`` of its kink: a kink that the two roundings straddle is an
O(1) change of one term of a weight gradient that no tolerance on
rounding covers, and at batch 512 -- twenty-five million ReLU
evaluations -- some always do, so that comparison runs in float64.  A
float32 weight gradient is a sum of thousands of terms that largely
cancel, whose rounding alone reaches ``1e-4`` of its largest entry, so
the float32 gradients are held to that rather than entry by entry; the
float64 ones are held entry by entry.
"""
...
from discopy.neural import fused, fused_cuda            # noqa: E402
...
def both(rows, dtype, backend, init=False, widths=HALF):
    """ The reference round and a fused round on the same random input. """
    ...
    found = round_of(fused.step_of(cmap, routing, backend), x, init, params)
    return reference, found


@pytest.mark.parametrize("backend", BACKENDS)
@pytest.mark.parametrize("rows", [1, 7, 512])
def test_forward_matches_reference(rows, backend):
    (out, _, groups), (found, _, found_groups) = both(
        rows, torch.float32, backend, init=rows == 7)
...
def test_gradients_match_reference(rows, dtype, backend):
    (_, grads, _), (_, found, _) = both(rows, dtype, backend, init=rows == 7)
...
@pytest.mark.parametrize("backend", BACKENDS)
def test_backward_is_exact(backend):
    ...
    x = torch.randn(2, geo.total, device=DEVICE, dtype=torch.float64,
                    requires_grad=True)
    function, prec = (fused.FusedRound, "ieee") if backend == "triton" \
        else (fused_cuda.FusedRound, "fp64")

    def once(x, *params):
        return function.apply(
            x, None, routing["perm_inverse"], geo, prec, *params)
    assert torch.autograd.gradcheck(
        once, (x, *params), eps=1e-6, atol=1e-5, rtol=1e-4)


@pytest.mark.parametrize("backend", BACKENDS)
def test_no_grad_keeps_nothing(backend):
    """ Under ``no_grad`` the round allocates its output and nothing else. """
    _, cmap, routing = build()
    step = fused.step_of(cmap, routing, backend)
...
def test_other_maps_keep_the_reference_step():
    ...
    routing = cmap._device_routing(DEVICE)
    assert fused.step_of(cmap, routing) is None
    assert fused.step_of(cmap, routing, "cuda") is None
    step = cmap.compile_fused(backend="cuda")._step_body(DEVICE)
    ...
    _, cmap, routing = build()
    x = torch.randn(2, fused.geometry(cmap).total, device=DEVICE)
    with pytest.raises(ValueError, match="backend"):
        fused.step_of(cmap, routing, "cutlass")(x, None, None)


#: What :func:`round_of` returns the gradients of, in order.
NAMES = ("x", "init", "w1", "b1", "w2", "b2", "wih", "whh", "bih", "bhh",
         "gamma", "beta", "we", "be", "wphi", "bphi", "wr1", "br1", "wr2",
         "br2")

#: The gradients that pass through a ReLU mask: the input's and the first
#: layer's of the encoder and of the unit's two networks.
MASKED = {"x", "w1", "b1", "wphi", "bphi", "wr1", "br1"}


def test_tensor_cores_round_like_cublas():
    """
    The CUDA kernels under ``"high"`` precision -- TF32 tensor cores --
    against the same kernels under ``"highest"``, and against the Triton
    kernels under ``"high"``: the forward within ``2e-3`` relative, the
    rounding of a 10-bit mantissa, and the weight gradients within
    ``1e-3`` of their largest entry, except those a ReLU mask gates: a
    TF32 pre-activation sits ``1e-3`` from its full-precision value, a
    row of the lattice evaluates some thirty thousand ReLUs, so some
    always straddle their kink between the two precisions, and each one
    that does flips an O(1) term of the first-layer gradients (and of the
    input's), which are held to ``5e-2`` of their largest entry instead.
    """
    net, cmap, routing = build()
    x = clear_of_kinks(cmap, 64, torch.float32)
    params = list(net.map.ar.parameters())

    def run(backend, precision):
        torch.set_float32_matmul_precision(precision)
        return round_of(fused.step_of(cmap, routing, backend), x, None,
                        params)
    out, grads, _ = run("cuda", "highest")
    for backend in ("cuda", "triton"):
        found, found_grads, _ = run(backend, "high")
        torch.testing.assert_close(
            found, out, atol=2e-3 * out.abs().max().item(), rtol=2e-3)
        for name, grad, found_grad in zip(NAMES, grads, found_grads):
            if grad is None:
                continue
            bound = 5e-2 if name in MASKED else 1e-3
            assert (found_grad - grad).abs().max() \
                <= bound * grad.abs().max(), (backend, name)


def test_kernels_stay_in_registers():
    """ No CUDA kernel spills at the tensor-core precision. """
    _, cmap, _ = build()
    run = fused_cuda.kernels(fused.geometry(cmap), DEVICE)
    for kernel in fused_cuda.KERNELS:
        attributes = run.attributes(kernel, "tf32")
        assert attributes["regs"] <= 128 and attributes["local"] == 0, \
            (kernel, attributes)


def test_cubin_cache_is_hit():
    """ The second build of a geometry reads the cubin from the cache. """
    _, cmap, _ = build()
    geo = fused.geometry(cmap)
    fused_cuda.Kernels(geo, DEVICE).build("tf32")
    again = fused_cuda.Kernels(geo, DEVICE)
    again.build("tf32")
    assert again.cached["tf32"]
    assert any(fused_cuda.CACHE.glob("*.cubin"))
...
def losses(fused_round: bool, backend: str = "triton", steps: int = 5):
    ...
    cfg = lattice_train.Config(
        ...
        fast=True, fused=fused_round, fused_backend=backend)
...
@pytest.mark.parametrize("backend", BACKENDS)
def test_training_step_agrees(backend):
    """ Five trainer iterations, the fused round off and on. """
    reference, found = losses(False), losses(True, backend)
