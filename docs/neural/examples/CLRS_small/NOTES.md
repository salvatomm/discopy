...
## What Phases 1 and 2 measured, and where it lives

The gate passed (`artifacts/floor-replication.md`): the 2022 recipe
reproduces four of five published rows within noise, and the fifth
(`dijkstra`) was diagnosed rather than excused — the published number is
an upper-tail draw of a high-variance recipe under a dense selection
cadence, measured with two one-axis arms.  R-vs-O replicates inside the
reference for `dijkstra`, `mst_prim`, `bellman_ford` and `bfs`; the
`floyd_warshall` prediction was **refuted** (no-hint helps there too),
so our FW ID-collapse-without-hints is a fact about the open-loop
edge-state model and is filed as one.  The decomposition
(`artifacts/phase2-decomposition.md`) retired two suspects with
measurements (ties on `bellman_ford`: exactly zero; the per-sample
output freeze: nothing) and retired the `bfs` mystery outright — the
hint gradient was ~9 of its 10 points, output-only training lands a
point from the local floor.  What remains is `bellman_ford`'s 19
points, with the floor's 25× better distance hints as the mechanism
candidate T-D now tests.

Two reporting rules these phases forced.  A single-seed reading is a
*draw*, not a level — arm B's celebrated 0.9296 was seed 0 of a
0.8942 ± 0.0257 arm — and a published anchor is a draw too, which is
why `dijkstra` comparisons now prefer the local floor with its six-seed
spread.

## The closed loop is the model's own forward pass, not a training trick

T-D (`artifacts/td-design.md`) adds `Budget.feedback`: at every
checkpoint boundary the node input loop is rewritten as encoded inputs
plus encoded hints — ground truth w.p. `forcing` at train, the model's
own hard-decoded hints otherwise and always at evaluation.  It lives in
`Model.refeed` plus a branch of `Model.run` that steps segment by
segment on the resumption law; `discopy.neural` is untouched, and the
segments retain exactly the checkpoints the loss reads.  Two honest
consequences: every evaluation of a `feedback` model is closed-loop
too (that is its forward semantics, not an eval-time choice), and the
residual curve of such a model is not comparable with an open-loop
one's, since the state is externally rewritten every `HOPS` rounds.
...
## T-D came back negative, and coherently so

The closed loop's triage (`artifacts/td-results.json`) moved
`bellman_ford` by one point (0.6873 ± 0.0125 against arm R's
seed-matched 0.677, gate at +3) and left the distance-hint MSE exactly
where every open-loop arm has it — 0.0327 against the floor's 0.0012.
Score and mechanism metric moved together, which is what the
pre-registration demanded of a negative: hint re-encoding and noisy
teacher forcing are *eliminated* as the floor-gap mechanism, not merely
unsupported.  The `bfs` control read the same lesson from the other
side: closed-loop-with-hint-loss sits at R's 0.894, so closing the
loop is not what makes hint supervision benign for the reference
either.  Speculative seeds 1–2, launched while the triage eval ran,
were killed on the gate (R5).

## T-C rides machinery Part 2 already built

The last ranked suspect is the wiring: the reference MPNN runs on the
complete graph (`processors.py:515-521` overwrites `adj_mat` with
ones) and reads the sampled graph as edge *data*, ours runs on the
sampled graph itself.  The complete-graph diagram, `adj`-as-feature,
`dense_graph`, `pair_sites` and the edge-pointer decode over all pairs
all exist already for `floyd_warshall`; T-C
(`artifacts/tc-design.md`) only had to make membership of
`dataset.DENSE` a run-time fact.  `DENSE` became a **set** mutated in
place by `dataset.densify` — in place because `model.py` holds a
`from dataset import DENSE` binding a rebind would silently miss —
plus `Budget.dense`, tag part `dense`, and `--dense` on the two entry
points.  The arm changes the diagram of a sample and nothing else:
open loop, hint loss, HOPS, optimiser and budget all stay at arm R's
values, and the parameter delta is one scalar edge encoder.

## The wiring was the gap

T-C came back positive at three seeds and closed Phase 3
(`artifacts/tc-results.json`).  Putting the processor on the complete
graph and demoting the sampled graph to edge data — the one axis the
reference always had and we never did — moves `bellman_ford` from
arm R's 0.6783 ± 0.0024 to **0.9482 ± 0.0050** OOD, twenty-seven
points on a twenty-four-point gap, suggestively *above* the published
floor (0.9201, t = 4.91 at the df = 2 critical 4.30).  The `bfs`
control refused to be a control: **1.0000** through the ×3 depth
sweep, above the published Triplet-GMPNN ceiling, which re-reads
Phase 2's hint-gradient damage as a tax levied only on sparse
diagrams.  The pre-registered mechanism story died on schedule while
the arm succeeded: the distance-hint MSE never moved (~0.025, twenty
times the floor's 0.0012) while the pointer-hint accuracy went 0.44 →
0.95 on every seed.  `argmin_v d(v) + w(v, u)` wants a ranking, not a
regression; the floor's 25×-better `d` was a correlate of its wiring,
not the cause of its pointer.  What dense wiring actually buys is
reach: on the sampled graph the per-round propagation radius is a
constant of the topology while OOD trajectories lengthen; on the
complete graph every node hears every extremum every round.  Both
gates, both controls, and the stopping rule agree — nothing about the
floor gap is unexplained.
